...
class Comment(object):
    ...
    def __eq__(self, other):
        return (isinstance(other, Comment) and self.line_no == other.line_no
                and self.column_no == other.column_no
                and str(self) == str(other))

    def is_inline(self):
        return (not isinstance(self.token_before, yaml.StreamStartToken)
                and self.line_no == self.token_before.end_mark.line + 1 and
...
def comments_between_tokens(token1, token2):
    ...
    elif (token1.end_mark.line == token2.start_mark.line
          and not isinstance(token1, yaml.StreamStartToken)
          and not isinstance(token2, yaml.StreamEndToken)):
        return
    else:
        buf = token1.end_mark.buffer[token1.end_mark.pointer:token2.start_mark.
                                     pointer]
    ...
    for line in buf.split('\n'):
        pos = line.find('#')
        if pos != -1:
            comment = Comment(line_no, column_no + pos, token1.end_mark.buffer,
                              pointer + pos, token1, token2, comment_before)
            yield comment
...
def token_or_comment_or_line_generator(buffer):
    ...
    while tok_or_com is not None or line is not None:
        if tok_or_com is None or (line is not None
                                  and tok_or_com.line_no > line.line_no):
            yield line
            line = next(line_gen, None)
        else:
            yield tok_or_com
