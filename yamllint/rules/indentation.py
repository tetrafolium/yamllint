...
def _check(conf, token, prev, next, nextnext, context):
    ...
    first_in_line = (is_visible
                     and token.start_mark.line + 1 > context['cur_line'])
    ...
    if first_in_line:
        ...
        if isinstance(token,
                      (yaml.FlowMappingEndToken, yaml.FlowSequenceEndToken)):
        ...
        if found_indentation != expected:
            yield LintProblem(
                token.start_mark.line + 1, found_indentation + 1,
                'wrong indentation: expected %d but found %d' %
                (expected, found_indentation))

    if (isinstance(token, yaml.ScalarToken)
            and conf['check-multi-line-strings']):
    ...
    if isinstance(token, yaml.BlockMappingStartToken):
        ...
        if isinstance(next, (yaml.AnchorToken, yaml.TagToken)):
            if (next.start_mark.line == prev.start_mark.line
                    and next.start_mark.line < nextnext.start_mark.line):
                next = nextnext

        # Only if value is not empty
        if not isinstance(next, (yaml.BlockEndToken, yaml.FlowMappingEndToken,
                                 yaml.FlowSequenceEndToken, yaml.KeyToken)):
            if context['stack'][-1].explicit_key:
                ...
                indent = next.start_mark.column
                    ...
                    next,
                (yaml.BlockSequenceStartToken, yaml.BlockEntryToken)):
                ...
                if context['indent-sequences'] is False:
                    ...
                    if (context['spaces'] == 'consistent'
                            and next.start_mark.column -
    ...
    while True:
        if (context['stack'][-1].type == F_SEQ
        ...
        elif (context['stack'][-1].type == B_ENT
            ...
            context['stack'].pop()
              ...
              and isinstance(next,
                             (yaml.BlockEntryToken, yaml.BlockEndToken))):
            context['stack'].pop()
            ...
            context['stack'].pop()
              ...
              and isinstance(next,
                             (yaml.BlockEndToken, yaml.FlowMappingEndToken,
                              yaml.FlowSequenceEndToken, yaml.KeyToken))):
