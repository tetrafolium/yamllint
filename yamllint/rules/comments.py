...
def check(conf, comment):
    if (conf['min-spaces-from-content'] != -1 and comment.is_inline()
    ...
    if conf['require-starting-space']:
        ...
        while (comment.buffer[text_start] == '#'
               and text_start < len(comment.buffer)):
            text_start += 1
        if text_start < len(comment.buffer):
            if (conf['ignore-shebangs'] and comment.line_no == 1
                    and comment.column_no == 1
                    and re.match(r'^!\S', comment.buffer[text_start:])):
                return
            ...
            # \r itself is a valid newline on some older OS.
                ...
                yield LintProblem(comment.line_no, column,
